...
export async function GET(req: NextRequest) {
   ...
   try {
      ...
      do {
         ...
         const existingLinks = await mpLinksDb
            .find({ _id: { $in: filtered.map(m => m.id) } })
            .map(d => d._id)
            .toArray();
         filtered
            .filter(m => !existingLinks.includes(m.id))
            .forEach(m => {
               console.log(convertTime((m.end_time.getTime() - m.start_time.getTime()) / 1000), m.id, m.name);
         ...
         for (const match of matchType.lobby.sort((a, b) => a.end_time.getTime() - b.end_time.getTime())) {
            ...
            if (result) addLinks.push({ _id: match.id });
         }
         added = addLinks.length > 0;
         if (added) console.log("History result", await mpLinksDb.insertMany(addLinks));
      } while (fetchCount++ < 2 && added && cursorString);
      console.log(`Completed fetch. Added: ${added}, Cursor: ${cursorString}, Fetches: ${fetchCount}`);
